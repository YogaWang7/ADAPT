...
# @Author  :
# @File    : SA_modified.py
# @Description : Functions to perform Simulated Annealing.
...
class SimulatedAnnealingBase(SkoBase):
    ...
    other_params : array
         added. Other parameters that func may use
    n_dim : int
    ...
    def run(self):
        ...
        while True:
            ...
            for i in range(self.L):
                ...
                # : add 'self.other_params' to pass necessary parameters
...
class SimulatedAnnealingValue(SimulatedAnnealingBase):
    ...
    def __init__(self, func, x0, other_params,T_max=100, T_min=1e-7, L=300, max_stay_counter=150, **kwargs):
        ...
        # : why have to '* np.ones(self.n_dim)' cause nothing changes here.
...
class SAFast(SimulatedAnnealingValue):
    ...
    def cool_down(self):
        # : self.quench=1， self.c = 1/e
        # : By default, self.T = self.T_max * e^(-1/e*self.iter_cycle)
...
class SABoltzmann(SimulatedAnnealingValue):
    ...
    def get_new_x(self, x):
        ...
        # : in our case, std = b = self.hop / 3.0 / self.learn_rate mostly
        # : !Alert! When a is less than any number in b, a becomes std and std is always in shape of b
        std = np.where(a < b, a, b)
        # : xc is a noise. original expression:
        # xc = np.random.normal(0, 1.0, size=self.n_dim),
        # : in our case, std * self.learn_rate = self.hop / 3.0 / self.learn_rate * self.learn_rate = (self.hop / 3.0)
        # : self.learn_rate is USELESS!! So the final expression: x_new = x + xc*(self.hop / 3.0)
        #  altered:
    ...
    def cool_down(self):
        # : T goes down slower than SACauchy due to np.log() here
...
class SACauchy(SimulatedAnnealingValue):
    ...
    def get_new_x(self, x):
        ...
        # : In our case, self.T is super big.
