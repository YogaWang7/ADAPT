...
def get_model_save_path(flag,seed):
    model_params_MLP = ""
    if flag:
        model_params_MLP = opt.net_root_path + "NN_params_infer_artificial_v2_" + "noise=" + str(opt.noise_pct) + "_seed=" + str(
            seed) + ".pth"
    else:
        model_params_MLP = opt.net_root_path + "NN_params_infer_seed=" + str(seed) + ".pth"
...
def f_ts(x, alpha, device):
    ...
    :return:
    '''
    # -alpha*x > 85 will lead to an overflow.
    y2 = (1-torch.exp(torch.clamp(-alpha*x,max=torch.tensor(85.,device=device))))
...
def f_Equi_ts_log_vmap(t, v, d, b, alpha, labda):
...
def get_U_GT2_ts_log_vmap(LEN, T, v, d, b, alpha, labda, eps = 1e-30, device = device):
    ...
    # log(1) = 0.
    U_head = torch.tensor([0.,0.],requires_grad=True,device=device)  # U: the prob. that someone offers a bid in t_th round
    ...
    U_T_1 = torch.log(torch.tensor([eps],requires_grad=True,device=device))  # u[T+1]=0 means the auction cannot reach (T+1)-th round.
...
def loss_fn_3(input_data, Alpha, Labda, Target_data, eps, device):
    ...
    for i in range(len(input_data)):
        ...
        # U_i[LEN+1] will be visited when we cal P[LEN].
...
if __name__ == '__main__':
    ...
    for i in running_times:
        ...
        total_vali_metric,GT_metric = trainer(train_loader, val_loader, test_loader, model, opt, device)

        print(f"========== IN Vali dataset, NN Model:  {total_vali_metric.detach().cpu().numpy()} ==========")
